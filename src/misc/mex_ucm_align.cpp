        		  ...
        		  int nrhs, const mxArray*prhs[] )
{
    ...
    std::size_t nr_project =           mxGetScalar(prhs[4]);
    std::size_t mode       =           mxGetScalar(prhs[5]);

    /* Check sizes */
    if (ucm2.rows() != 2*lp_ucm.rows()+1 || ucm2.rows() != 2*lp_project.rows()+1)
        mexErrMsgTxt("UCM and partitions sizes not coherent");
            
    /* Project the superpixels of lp_project to lp_ucm via maximum overlap */
    std::vector<std::vector<std::size_t> > sup_count(nr_project,std::vector<std::size_t>(nr_ucm,0));
    for(std::size_t xx=0; xx<lp_ucm.rows(); ++xx)
        for(std::size_t yy=0; yy<lp_ucm.cols(); ++yy)
            sup_count[lp_project(xx,yy)-1][lp_ucm(xx,yy)-1]++;
 
    
    /* Map between the labels of lp_project to lp_ucm */
    std::vector<label_type> sup_map(nr_project,0);
    for(std::size_t ii=0; ii<nr_project; ++ii)
        for(std::size_t jj=1; jj<nr_ucm; ++jj)
            if (sup_count[ii][sup_map[ii]]<sup_count[ii][jj])
                sup_map[ii] = jj;

    /* Relabel lp_project */
    Eigen::Array<label_type,Eigen::Dynamic,Eigen::Dynamic> lp_proj_rel(lp_project.rows(),lp_project.cols());
    for(std::size_t xx=0; xx<lp_project.rows(); ++xx)
        for(std::size_t yy=0; yy<lp_project.cols(); ++yy)
            lp_proj_rel(xx,yy) = sup_map[lp_project(xx,yy)-1]+1;
...
// }
    ...
    contour_container conts = part2cont(lp_proj_rel);
// printf("Conts:\n");    
// conts.print();
...
//     /* Invert the map (set of labels from lp_ucm that overlap with each label) */
//     std::vector<std::list<label_type> > inv_sup_map(nr_ucm);
//     for(std::size_t ii=1; ii<nr_project; ++ii)
//         inv_sup_map[sup_map[ii]].push_back(ii);
...
// ms.print();
    ...
    plhs[0] = mxCreateDoubleMatrix(ucm2.rows(),ucm2.cols(),mxREAL);
